...
var gridPointsByPos = [];
var gridPoints = [];
var wallSet = new Set(); //used to store the start and endPoint during resets, etc. 

var startPoint;
var endPoint;
var mode = null; //any point in 2D space
...
class PathFindingAlg {
  ...
  findPath() {
    ...
    while (openSet.size > 0) {
      ...
      closedSet.add(currentNode); //might need to put this after getNighbors.... then replace closedSet.hasIn(neighborNode with currentNode
      ...
      if (currentNode.id == endNode.id) {
        retracePath(startNode, endNode); //hit the last point, exit's the loop.
  ...
  createGrid() {
    ...
    for (var i = 0; i < this.width; i += NODESIZE) {
      ...
      for (var j = 0; j < this.height; j += NODESIZE) {
        gridPointsByPos[i][j] = countNodes; //here's the problem , need to set the walkability of the node without always being true...
...
grid.createGrid();
var myPath = new PathFindingAlg(grid, startPoint, endPoint); //distance from a node to  another node
